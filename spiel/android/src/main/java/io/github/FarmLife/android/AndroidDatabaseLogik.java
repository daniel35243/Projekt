...
public class AndroidDatabaseLogik implements DatabaseLogik {
    ...
    public void updateGame(int coins, int level, int xp, int tag, int uhr) {
        db.updateGame(coins, level, xp, tag, uhr);
    }
    ...
    public GameByID getGame(int ID) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            int Tag = cursor.getInt(cursor.getColumnIndexOrThrow("Tag"));
            int Uhr = cursor.getInt(cursor.getColumnIndexOrThrow("Uhr"));
            result = new GameByID(ID, Coins, Level, XP, Tag, Uhr);
            cursor.close();
        }
