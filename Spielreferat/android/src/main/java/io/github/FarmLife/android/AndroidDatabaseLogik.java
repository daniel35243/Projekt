...
public class AndroidDatabaseLogik implements DatabaseLogik { // Verbindung zu Core

    private final io.github.FarmLife.android.DatabaseHelper db; // Datenbank Instanz

    public AndroidDatabaseLogik(Context context) {
        db = new io.github.FarmLife.android.DatabaseHelper(context); // Zugang zu Android
    }
    ...
    public InventorySlotDB getInventorySlot(int slot) {
        Cursor cursor = db.getInventorySlot(slot); // Liest alle Daten des Slots aus
        InventorySlotDB result = null; // Wenn nichts gefunden wird wird null zurückgegeben

        if (cursor != null && cursor.moveToFirst()) { // Kein fehler aufgetreten ist und auf erste zeile
            String item = cursor.getString(cursor.getColumnIndexOrThrow("item")); // Holt item aus Spalte
            int anzahl = cursor.getInt(cursor.getColumnIndexOrThrow("Anzahl")); // Holt Anzahl aus Spalte
            result = new InventorySlotDB(slot, item, anzahl); // objekt mit Daten erzeugt
            cursor.close(); // ressourcen freigeben
        }

        return result; // zurückgeben des Objekts
    }
